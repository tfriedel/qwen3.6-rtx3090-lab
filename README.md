...
**Levers that did *not* help on this rig** (don't bother — details in [`FINDINGS.md`](./FINDINGS.md)): TP=2 is the only decode-TPS lever (TP=4 is slower, not faster, on PCIe-only 3090s); `--cudagraph-mode FULL_DECODE_ONLY` — no win; sampler sweeps — no win; llama.cpp for the dense 27B — 3× slower than vLLM; **LMCache for prefill/KV-offload — no working path** (Qwen3.6 is a hybrid-Mamba model that vLLM's KV connectors can't run; the one connector that boots stores nothing). `tp2` already has vLLM prefix caching on, which covers the "re-submit the same large file" case for free.
